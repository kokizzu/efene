...
This section describes the language through simple working examples, for a complete reference see [language reference](language-reference.md).
...
As before we can use the else clause instead of matching with **\_\_**, notice that efene is smart enough to match all the arguments in the function, this means the else is "expanded" to "case \_, \_:" so you don't have to.
...
There's a module that you will use frequently and it's the [io module](http://erlang.org/doc/man/io.html)

the most useful function it has is the [io.format function](http://erlang.org/doc/man/io.html#format-1)
...
For a full list of them you can see [the documentation of io.format:2](http://erlang.org/doc/man/io.html#format-2).
...
the function dynamic_call receives no arguments and calls the function [lists.seq:2](http://www.erlang.org/doc/man/lists.html#seq-2) in any way it can.
...
If we had the arguments to the function in a list we can also call the function dynamically using [erlang.apply:3](http://www.erlang.org/doc/man/erlang.html#apply-3)
...
In the third example we use the builtin [erlang.throw:1](http://www.erlang.org/doc/man/erlang.html#throw-1) function to throw an exception of type throw and we immediately catch it and print what the error was.
...
For more details on operations described in this and the following sections see the [language reference](language-reference.md).
...
If you need them, then the syntax should be what you would expect, if you don't know what they are for you can come back later when you need them or check the [language reference](language-reference.md)
...
In the example above we use both ways, first we use the function [maps.get:2](http://www.erlang.org/doc/man/maps.html#get-2):
...
Let's see it line by line, first we get the process id (pid) of the current process by calling the builtin function [erlang.self:0](http://www.erlang.org/doc/man/erlang.html#self-0):

``` javascript
OuterPid = self()
```

Then we spawn a new process by calling the builtin function [erlang.spawn:1](http://www.erlang.org/doc/man/erlang.html#spawn-1):
...
At some point you may want to add some types to your code, for documentation purposes or to help tools like [dialyzer](http://www.erlang.org/doc/man/dialyzer.html) spot bugs for you, efene as usual provides the same features as erlang to add types to your code, let's see an example:
...
For more details on the syntax see the [language reference](language-reference.md).

For more details on types see [Erlang's Types and Function Specifications](http://www.erlang.org/doc/reference_manual/typespec.html) documentation about it.
